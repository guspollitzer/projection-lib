...
trait PiecewiseAlgebra {
	...
	trait Trajectory[+A] {
		...
		def richMap[B](f: (index: Int, startingInstant: Instant, endingInstant: Instant, wholePieceIntegral: A) => B): Trajectory[B]

		def combineWith[B, C](that: Trajectory[B])(f: (A, B) => C): Trajectory[C] = combine(this, that)(f)

		def richCombineWith[B, C](that: Trajectory[B])(f: (index: Int, startingInstant: Instant, endingInstant: Instant, a:A, b: B) => C): Trajectory[C] = richCombine(this, that)(f)
	...
	def buildTrajectory[A](f: (pieceIndex: Int) => A): Trajectory[A]

	def buildTrajectory[A](f: (pieceIndex: Int, pieceStart: Instant, pieceEnd: Instant) => A): Trajectory[A]

	def buildTrajectory[S, A](initialState: S)(valueBuilder: (state: S, index: Int, start: Instant, end: Instant) => A)(nextPieceInitialStateBuilder: (S, A) => S): Trajectory[A]

	def buildLazyTrajectory[S, A](initialState: S)(valueBuilder: (state: S, index: Int, start: Instant, end: Instant) => A)(nextPieceInitialStateBuilder: (S, A) => S): Trajectory[A]
	
	def combine[A, B, C](ta: Trajectory[A], tb: Trajectory[B])(f: (A, B) => C): Trajectory[C]

	def richCombine[A, B, C](ta: Trajectory[A], tb: Trajectory[B])(f: (index: Int, startingInstant: Instant, endingInstant: Instant, a: A, b: B) => C): Trajectory[C]
}
